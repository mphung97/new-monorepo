...
function handleACPKCESigninRequest(req, res) {
  ...
  datastore
    ...
    .then(() => datastore.runQuery(clientQuery))
    .then(result => {
      if (result[0].length === 0) {
    ...
    .then(() => {
      ...
      return Promise.all([
        datastore.upsert({ key: codeKey, data }),
    ...
    .then(result => {
      ...
      if (entry.client_id !== clientId) {
        return Promise.reject(
          new Error("Client ID does not match the record.")
        );
      }

      if (entry.redirect_url !== redirectUrl) {
        return Promise.reject(
          new Error("Redirect URL does not match the record.")
        );
      }
      ...
      if (codeVerifier !== undefined && entry.code_challenge !== undefined) {
        ...
        if (codeChallenge !== entry.code_challenge) {
          return Promise.reject(
            new Error("Code verifier does not match code challenge.")
          );
        }
      } else if (
        codeVerifier === undefined ||
...
function handleCCTokenRequest(req, res) {
  ...
  datastore.runQuery(clientQuery).then(result => {
    if (result[0].length === 0) {
      return res.status(400).send(
        JSON.stringify({
          error: "access_denied",
